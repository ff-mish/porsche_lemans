...
class TwitteAR extends CActiveRecord {
 ...
 public function beforeSave() {
  ...
  public function getListInLevel($level, $num = 40) {
    $query = new CDbCriteria();
    $params = array();
    $user = UserAR::crtuser();
    if (!$user) {
    ...
    if ($level == self::LEVEL_USER) {
      ...
      if ($from == UserAR::FROM_TWITTER) {
        $userAr = new UserAR();
        $porsche_user = $userAr->load_user_by_uuid(Yii::app()->params["twitter_uid"]);
        if ($porsche_user) {
        ...
        $userAr = new UserAR();
        $porsche_user = $userAr->load_user_by_uuid(Yii::app()->params["weibo_uid"]);
        if ($porsche_user) {
          $uids[] = $porsche_user->uid;
        }
      }
