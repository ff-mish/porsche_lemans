$(function ($) {
    if (!Detector.webgl) {
        ...
        $.ajax({ url: 'data/background.json', type: "GET", async: true, cache: false, dataType: "json", success: function (background) {
            globe.addBackground(background);
            $.ajax({ url: 'data/24h.json', type: "GET", async: true, cache: false, dataType: "json", success: function (ret) {
                if (ret && ret.status == 0) {
                    data = ret.data;
